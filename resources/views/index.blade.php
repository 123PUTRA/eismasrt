...
<div class="container d-flex flex-column align-items-center vh-100">
        ...
        <button type="button" class="btn btn-lg btn-dark dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
         ...
         <ul class="dropdown-menu">
            @foreach($events as $event)
                <li><a class="dropdown-item dropdown-item-1" href="{{ route('events.show', $event->id) }}">{{ $event->nama }}</a></li>
            @endforeach
        </ul>
    </div>
    ...
    <div class="btn-group" id="dropdown2">
        ...
        <ul class="dropdown-menu">
          @foreach($events as $event)
              <li><a class="dropdown-item dropdown-item-2" href="{{ route('events.peserta', $event->id) }}">{{ $event->nama }}</a></li>
          @endforeach
        </ul>
    </div>
</div>
